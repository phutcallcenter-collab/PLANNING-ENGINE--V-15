...
const LAST_BACKUP_KEY = 'control-puntos:last-backup'
const LAST_RESTORE_KEY = 'control-puntos:last-restore'
...
export function setLastBackupTimestamp(date: string): void {
    if (!isBrowser) return
    localStorage.setItem(LAST_BACKUP_KEY, date)
}

export function getLastRestoreTimestamp(): string | null {
    if (!isBrowser) return null
    return localStorage.getItem(LAST_RESTORE_KEY)
}

export function setLastRestoreTimestamp(date: string): void {
    if (!isBrowser) return
